...
$departmentId = (int)($currentUser['active_department_id'] ?? $_SESSION['active_department_id'] ?? $currentUser['department_id'] ?? 0);
$status = (string)($_GET['status'] ?? '');
$service = new DepartmentNotificationService($pdo);
$notifications = $departmentId > 0 ? $service->listForDepartment($departmentId, $status) : [];
...
<main class="content">
    <div class="page-header">
        <div>
            ...
            <p>Attention requests sent to your active department.</p>
        </div>
    </div>
    ...
    <?php if ($departmentId <= 0): ?>
        <div class="card alert-warning">
            Select an active department before viewing department notifications.
        </div>
    <?php else: ?>
        <div class="card">
                        ...
                        <tbody>
                            <?php foreach ($notifications as $notification): ?>
                                <tr>
                                    ...
                                    <td><?= e((string)($notification['from_department_name'] ?? 'Unknown')) ?></td>
                                    <td><?= nl2br(e((string)$notification['reason'])) ?></td>
