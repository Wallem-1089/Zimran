...
if ($visitId > 0) {
    ...
    $records = $radiologyService->listByVisit($visitId, $currentUser);
} elseif ($patientId > 0) {
    $visit = null;
    $records = $radiologyService->listByPatient($patientId, $currentUser);
} else {
...
<main class="content">
    <div class="page-header">
        ...
        <div class="form-actions">
            ...
            <?php else: ?>
                <a class="btn-secondary" href="../medical_records/chart.php?patient=<?= (int)$patientId ?>&tab=radiology">Patient Chart</a>
            <?php endif; ?>
        </div>
    </div>
                ...
                <table class="table">
                    ...
                    <tbody>
                        <?php foreach ($records as $record): ?>
                            <tr>
                                <td><?= (int)$record['id'] ?></td>
                                <td><?= e((string)($record['study_requested'] ?? $record['tests_requested'] ?? '')) ?></td>
