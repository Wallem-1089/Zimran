...
storeRequireMovementAccess($permissionService, $currentUser, 'adjust');
...
if (!csrfVerify($_POST['csrf_token'] ?? null)) {
    ...
    header('Location: adjust.php?id=' . (int)($_POST['inventory_item_id'] ?? 0));
    exit;
}

$result = $storeService->adjustStock($_POST, $currentUser);
if (($result['success'] ?? false) !== true) {
    $_SESSION['validation_errors'] = $result['errors'] ?? ['Unable to adjust stock.'];
    $_SESSION['old_store_movement'] = $_POST;
    header('Location: adjust.php?id=' . (int)($_POST['inventory_item_id'] ?? 0));
    exit;
}
