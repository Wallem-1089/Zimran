...
$result = $problemListService->markHistoryEnteredInError($id, (string)($_POST['reason'] ?? ''), (int)$currentUser['id'], (int)($_POST['version'] ?? 0), $visitId, longitudinalDepartmentId($currentUser)); longitudinalFlash($result, 'Medical history marked entered in error.'); header('Location: view.php?id=' . $id . longitudinalQuery($visitId)); exit;
